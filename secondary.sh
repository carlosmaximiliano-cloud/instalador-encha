...
exibir_menu() {
    ...
    while true; do
        ...
        printf "\n"
        centralizar "Sistema de Deploy Automatizado"
        printf "%b\n" "${amarelo}$(printf -- '=%.0s' $(seq 1 $(tput cols)))${reset}"
        ...
        for (( i=0; i<meio; i++ )); do
            ...
            local item_esq=$(printf "[ ${azul}%02d${reset} ] - %-25s %s" "$idx_esq" "$texto_esq" "$status_esq")
            ...
            if [ $idx_dir_calc -lt $max_items ]; then
                ...
                item_dir=$(printf "| [ ${azul}%02d${reset} ] - %-25s %s" "$idx_dir" "$texto_dir" "$status_dir")
            fi
        ...
        printf "%b\n" "${amarelo}$(printf -- '-%.0s' $(seq 1 $(tput cols)))${reset}"
        printf "      [ ${azul}05${reset} ] - %-25s | [ ${azul}11${reset} ] - %-25s | [ ${azul}12${reset} ] - %s\n" "${OPCOES[5]}" "${OPCOES[11]}" "${OPCOES[12]}"
        printf "%b\n" "${amarelo}$(printf -- '_%.0s' $(seq 1 $(tput cols)))${reset}"
        
        local largura=$(tput cols)
        local padding=$(( (largura - 70) / 2 )) 
        printf "      | --- Digite ${amarelo}P1${reset} para ir para página 1"
        printf " %*s" "$padding" ""
        printf "| Digite ${amarelo}P2${reset} para ir para página 2 -->\n"

        echo ""
        read -p "$(printf "%b" ${amarelo}"Digite o NÚMERO da opção desejada ou COMANDO oculto: "${reset})" opcao

        # --- Seu CASE original vai aqui ---
        case $opcao in
